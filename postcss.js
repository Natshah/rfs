  ...
  return css => {
    css.walkRules(rule => {
      ...
      rule.walkDecls(decl => {
        ...
        if (!decl.parent.selector.includes(DISABLE_RFS_SELECTOR) && new RegExp(opts.functionName + '(.*)', 'g').test(decl.value)) {
      ...
      if (mediaQueryRules.length !== 0) {
        ...
        rule.parent.insertAfter(rule, fluidMediaQuery);

        if (extraBlocks) {
          extraBlocks.forEach(disableBlock => {
            rule.parent.insertAfter(rule, disableBlock);
          });
        }
